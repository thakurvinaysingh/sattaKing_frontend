...
export default function AdminLogin() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (res.data.success && res.data.user?.userType === "admin") {
        login({ ...res.data.user, token: res.data.token });
        navigate("/admin/dashboard");
      } else {
        setError("Not authorized or not an admin");
      }
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#4E7584] via-[#63B0CD] to-[#4E7584]">
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          <label className="text-[#4E7584] font-medium" htmlFor="admin-username">
            Username
          </label>
          <input
            id="admin-username"
        ...
        <div className="flex flex-col gap-2 relative">
          <label className="text-[#4E7584] font-medium" htmlFor="admin-password">
            Password
          </label>
          <input
            id="admin-password"
